...
class EncoderGenerator_Res(nn.Module):
    ...
    def __init__(self, norm_layer='instance', image_size=128, input_nc=3, latent_dim=256):
        super(EncoderGenerator_Res, self).__init__()

        self.latent_dim = latent_dim
        latent_size = int(image_size / 8) # 16
        self.latent_size = latent_size
        # 128 * 16 * 16
        longsize = 128 * latent_size * latent_size
        ...
        layers_list.append(ResBlock(128, norm_type=norm_layer, padding_type=padding_type))
        self.conv = nn.Sequential(*layers_list)
    ...
    def forward(self, input):
        input = self.conv(input)
        ten = input.reshape(input.shape[0], -1) # batch * 8192
        #print("ten",ten.shape)
        mu = self.fc_mu(ten)
        return mu
...
class DecoderGenerator_image_Res(nn.Module):
    ...
    """
    def __init__(self, norm_layer='instance', image_size=128, output_nc=3, latent_dim=256):
