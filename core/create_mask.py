...
def get_mask(height, width, landmark):
    mask = np.zeros((height, width), np.float32)
    left_h,left_w = landmark[14,0],landmark[14,1]
    right_h,right_w = landmark[19,0],landmark[19,1]

    mask[left_h-16:left_h+16,left_w-16:left_w+16] = 1.0
    mask[right_h-16:right_h+16,right_w-16,right_w+16] = 1.0

    cv2.imshow('test',(mask * 255).astype('uint8'))
    return (mask * 255).astype('uint8')
